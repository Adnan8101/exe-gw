...
exports.default = {
    data: new discord_js_1.SlashCommandBuilder()
        ...
        .addStringOption(option => option.setName('duration').setDescription('Duration (e.g. 30s, 1m)').setRequired(true))
    ...
    async prefixRun(message, args) {
        ...
        if (args.length < 3) {
            return message.reply(`${emojis_1.Emojis.CROSS} Usage: \`!gstart <duration> <winners> <prize>\`\nExample: \`!gstart 10m 1 Nitro Classic\``);
        }
        /*
        ...
        */
        ...
        if (isNaN(winners)) {
            return message.reply(`${emojis_1.Emojis.CROSS} Invalid winner count.`);
        }
        await this.run(message, message.channel, durationStr, winners, prize);
    },
    async run(ctx, channel, durationStr, winners, prize) {
        const duration = parseDuration(durationStr);
        if (!duration) {
            const msg = `${emojis_1.Emojis.CROSS} Invalid duration. Use format: 30s, 1m, 2h`;
            if (ctx.reply)
        ...
        try {
            ...
            const successMsg = `${emojis_1.Emojis.TICK} Giveaway started in ${channel}!`;
            if (ctx.editReply)
            ...
            console.error(error);
            const failMsg = `${emojis_1.Emojis.CROSS} Failed to start giveaway.`;
            if (ctx.editReply)
                await ctx.editReply(failMsg);
            else
