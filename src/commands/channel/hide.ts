...
export default {
  data: new SlashCommandBuilder()
    .setName('hide')
    .setDescription('Hide a channel')
    .addChannelOption(option => option.setName("channel").setDescription("The channel to hide (defaults to current)").setRequired(false))
  ...
  async prefixRun(message: Message, args: string[]) {
    ...
    try {
      ...
      const embed = new EmbedBuilder()
        .setColor(Theme.ErrorColor)
        .setDescription(`${Emojis.TICK} Hidden ${channel}`)
      ...
      return message.reply({ embeds: [new EmbedBuilder()
        .setColor(Theme.ErrorColor)
        .setDescription(`${Emojis.CROSS} Failed to hide channel`)
      ]});
    }
  }
};
